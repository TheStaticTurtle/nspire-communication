...
int key_already_pressed_contactSearch = 0;
int key_already_pressed_contactLast = 0;
...
int main(void)
{
	...
	uart_printf("#$#CALC:ENTERING:MES\r\n");

	//Main loop
	while(!isKeyPressed(KEY_NSPIRE_ESC)){
		//Part where we send messages from the calc
		if(isKeyPressed(KEY_NSPIRE_CTRL) && !key_already_pressed_ctrl && !key_already_pressed_help && !key_already_pressed_contactSearch && !key_already_pressed_contactLast){
			//Test if the user has already selected an contact
			if(hasSelectedContact) {
				// Prompting the user
				nio_printf("<< ");
				char output[1024] = {0};
				nio_getsn(output,1024);
		...
		// Part for contact searching
		if(isKeyPressed(KEY_NSPIRE_C) && !key_already_pressed_help && !key_already_pressed_contactSearch && !key_already_pressed_ctrl && !key_already_pressed_contactLast){
			// Contact select part
			nio_clear(&csl);
			...
			key_already_pressed_contactSearch = 1;
		}
		// Part for last contact search
		if(isKeyPressed(KEY_NSPIRE_D)&& !key_already_pressed_help && !key_already_pressed_contactSearch && !key_already_pressed_ctrl && !key_already_pressed_contactLast){
			nio_clear(&csl);
			uart_printf("$#$GETLASTTHR:\r\n");
		...
		//Help message
		if(isKeyPressed(KEY_NSPIRE_H) && !key_already_pressed_ctrl && !key_already_pressed_help && !key_already_pressed_contactSearch && !key_already_pressed_contactLast){
			nio_clear(&csl);
                        nio_color(&csl,NIO_COLOR_BLACK,NIO_COLOR_WHITE);
                        nio_printf("                                                     ");
                        nio_printf("                      Help :                         ");
			...
			nio_puts("\t C     -> Select an contact in all contact");
			nio_puts("\t D     -> Select an contact in last sended");
			nio_puts("");
		...
		if(!any_key_pressed()) {
			...
			key_already_pressed_contactSearch = 0;
			key_already_pressed_contactLast = 0;
